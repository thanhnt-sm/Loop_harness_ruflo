<!-- POINTER: canonical source HLK/skills/glm/SKILL.md -->

---
name: glm
description: Use the active model as a lean planner and reviewer while GLM-5.2 executes concrete software-engineering work. Free tier (200K context).
...
Act as the control plane for the user's software-engineering task. The currently active model is the orchestrator. Keep it active for planning, decisions, and review; delegate implementation to the `glm-executor` subagent, which is pinned to GLM-5.2 (free tier, 200K context).
...
Use exactly one executor by default. Do not create speculative planners, researchers, reviewers, or parallel implementations. A single focused GLM-5.2 session is the standard path because every extra subagent adds cost and context duplication.
...
If `glm-executor` is unavailable, do not silently substitute another model. Report that the custom profile is missing or disabled.
