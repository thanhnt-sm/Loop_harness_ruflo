...
def verify(patched_files: list[str]) -> bool:
    ...
    for pf in py_files:
        f = update_common.REPO_ROOT / pf
        if not f.exists():
            continue
        code, out, err = run_cmd([sys.executable, "-m", "py_compile", str(f)], cwd=update_common.REPO_ROOT)
        if code != 0:
            print(f"[FAIL] py_compile {pf}: {err or out}")
            return False

    # import smoke test
    code, out, err = run_cmd([sys.executable, "tools/import_smoke_test.py"], cwd=update_common.REPO_ROOT)
    if code != 0:
        print(f"[FAIL] import smoke test: {err or out}")
        return False

    # qa_doc_audit
    code, out, err = run_cmd([sys.executable, ".devin/scripts/qa_doc_audit.py"], cwd=update_common.REPO_ROOT)
    if code != 0:
    ...
    for script in [
        ...
        code, out, err = run_cmd(script, cwd=update_common.REPO_ROOT, timeout=180)
        if code != 0:
