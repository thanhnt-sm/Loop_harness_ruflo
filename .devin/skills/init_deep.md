---
name: init_deep
description: "Use when a repository is too large to explain from memory, or when the shape of the codebase has changed. Generates hierarchical AGENTS.md context files near the code that needs them, giving future agents landmarks before they edit."
---
...
- **Never duplicate the root canon.** Local files contain *local* context only. Global
  rules live in `.devin/canon/` and are synced by the deployer, not by init_deep.
- **Don't write AGENTS.md inside `.devin/canon/` or `core/assets/`.** Those are the
...
`init_deep` writes the **project context** layer — stable, structural, directory-bound. It complements (not replaces) the three-layer runtime memory (hot/knowledge/cold). See `.devin/canon/MEMORY_PROTOCOL.md`. It's the *spatial* memory: "where am I in the codebase" without reading 50 files.
