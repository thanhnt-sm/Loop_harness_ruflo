---
name: memory-audit
description: "Use every 5 iterations, when scope changes, or before session end. Invokes .devin/scripts/memory_audit.py to review candidate memories, prune stale ones, and distills reusable patterns into knowledge_distill.md."
---
...
1. Call `python .devin/scripts/memory_audit.py --session <session_id>`.
2. The script does the following:
   - Read `.devin/session_state/<session_id>/candidate_memory.jsonl`.
   ...
   - If `.agents/knowledge_distill.md` exceeds 8KB, run a distillation pass:
     ...
     - archive evicted entries to `.devin/loop_state_archive.md`
   - Clear the processed candidates from `.devin/session_state/<session_id>/candidate_memory.jsonl`.
3. You may also read the audit output from the script and append any high-level judgment to `.agents/handoff_letter.md`.

## Output
```
## Memory audit
- script: python .devin/scripts/memory_audit.py --session <session_id>
- accepted: N
- discarded: N
